...
You could also combine the inventory files for the Solr and Zookeeper nodes into one file. However, to do so you need to define which nodes belong in which host group (`solr` vs. `zookeeper`) by adding those host groups to the end of the file
...
$ ansible-playbook -i common-utils/inventory/osp/openstack -e "{ \
        ...
        application: solr, cloud: osp, tenant: labs, project: projectx, domain: preprod, inventory_type: dynamic, \
...
$ ansible-playbook -i common-utils/inventory/aws/ec2 -e "{ \
        ...
        application: solr, cloud: aws, tenant: labs, project: projectx, domain: preprod, inventory_type: dynamic, \
        ansible_user: cloud-user, private_key_path: './keys', data_iface: eth0, api_iface: eth1, \
        solr_data_dir: '/data' \
    }" site.yml
```
